---
title: "Analisis Regresi - Praktikum 2"
...
data <- read_excel("C:\\Users\\qanit\\Downloads\\NSC-2025.xlsx", sheet="Sheet2")
head(data)
```
...
```{r}
(p_Fhit <- 1-pf(Fhit, dbReg, dbg, lower.tail = T))
```
...
$$\hat{Y} = 60.25436 + 0.18280 X_4$$

dengan Y adalah indeks pembangunan manusia dan X6 adalah persentase akses sanitasi layak.

Didapatkan nilai $\hat\beta_0 = 60.25436$ artinya nilai dugaan rataan indeks pembangunan manusia ketika persentase akses sanitasi layak 0 persen adalah 60.254%, akan tetapi nilai 0 tidak ada dalam selang pengamatan persentase akses sanitasi layak sehingga $\hat\beta_0$ tidak dapat didefinisikan lebih lanjut

Nilai $\hat\beta_0 = 0.18280$ artinya nilai dugaan perubahan dugaan rataan Indeks Pembangunan Manusia jika persentase akses sanitasi layak berubah satu tahun adalah sebesar 0.129%

Koefisien determinasi yang dihasilkan adalah 0.2328  atau 23.28% artinya keragaman indeks pembangunan manusia yang dapat dijelaskan oleh persentase akses sanitasi layak adalah sebesar 23.28%, sisanya yaitu 76.72% dijelaskan oleh peubah lain yang tidak dimasukkan dalam model.
