...
```{r}
library(readxl)
dt <- read_xlsx("C:\\Users\\qanit\\Downloads\\Data Analisis Regresi.xlsx")
dt
```

```{r}
y <- dt$"Persentase Rumah Tangga yang Memiliki Akses Terhadap Hunian Yang Layak"
x <- dt$"Persentase Rumah Tangga menurut Provinsi, Klasifikasi Desa, dan Jenis Lantai Terluas yang Layak (Persen)"
n <- length(dt)
```
...
Berdasarkan perhitungan, didapat dugaan persamaan regresi linear sebagai berikut. Dugaan persamaan tersebut dapat pula disebut sebagai nilai harapan dari peubah respons Y (Persentase Rumah Tangga yang Memiliki Akses Terhadap Hunian Yang Layak)

$$E[\hat{Y}] = \hat{Y} = -205.7691 + 2.7475 X$$
...
$$JKT = JKR + JKG$$
Nilai penguraian keragaman ini dapat diamati pula menggunakan fungsi anova pada bagian **Sum Sq (Jumlah Kuadrat)** seperti yang ditampilkan di bawah. Pada output sintaks di bawah, JKR model bernilai 3279.5 dan JKG model bernilai 6251.1

```{r}
(anova.model <- anova(model))
```

Dari tabel ANOVA dan penguraian keragaman, dapat diduga pula ragam galat/error dari model yang kita peroleh. Secara umum, error dari model dapat diduga oleh nilai Kuadrat Tengah Galat (KTG) sehingga berdasarkan output sintaks di atas, diketahui bahwa ragam galat dari model bernilai 168.9493. Hubungan antara keduanya dapat dituliskan sebagai berikut.
...
Oleh karena simpangan baku merupakan akar kuadrat dari ragam, maka nilai dugaan galat baku model yang kita bentuk adalah $S_e = \sqrt{168.9493} \approx 12.99805$
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [-364.3261 , -47.21207] dan untuk parameter $β_1$ adalah [1.144481 ,  4.350546]
...
```{r}
amatan.diduga <- data.frame(x=99)
predict(model, amatan.diduga, interval = "confidence")
```

Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=99 adalah 66.23477. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=10 berada dalam selang 61.98682 hingga 70.48271.
...
Misalkan kita ingin menduga nilai individu amatan ketika nilai x=99. Kita dapat memanfaatkan fungsi `predict` untuk mendapatkan nilai dugaan dan selang kepercayaannya sebagai berikut.

```{r}
predict(model, amatan.diduga, interval = "prediction")
```

Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=99 adalah 66.23477. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=99 berada dalam selang 39.55783 hingga 92.9117.